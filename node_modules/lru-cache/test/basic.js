var test = require("tap").test, LRU = require("../");
...
test("max", function(t) {
    var cache = new LRU(3); // test changing the max, verify that the LRU items get dropped.
    cache.max = 100;
...
test("weighed length item too large", function(t) {
    ...
    t.equal(cache.max, 10); // should fall out immediately
    cache.set("key", { val: "value", size: 50 });
    t.equal(cache.length, 0);
...
test("set returns proper booleans", function(t) {
    ...
    t.equal(cache.set("a", "A"), true); // should return false for max exceeded
...
test("pop the least used item", function(t) {
    var cache = new LRU(3), last;
    cache.set("a", "A");
    cache.set("b", "B");
    cache.set("c", "C");
    t.equal(cache.length, 3);
    t.equal(cache.max, 3); // Ensure we pop a, c, b
    cache.get("b", "B");
    last = cache.pop();
    t.equal(last.key, "a");
