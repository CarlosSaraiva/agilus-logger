...
module.exports = function(stream, options, done) {
    ...
    var encoding = options.encoding !== true
        ? options.encoding
        : "utf-8"; // convert the limit to an integer
    var limit = null;
    if (typeof options.limit === "number")
        limit = options.limit;
    if (typeof options.limit === "string")
        limit = bytes(options.limit); // convert the expected length to an integer
    var length = null;
    if (options.length != null && !isNaN(options.length))
        length = parseInt(options.length, 10); // check the length and limit options.
    ...
    return defer; // yieldable support
    function defer(fn) {
        done = fn;
    }
