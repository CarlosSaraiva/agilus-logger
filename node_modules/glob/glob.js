...
Glob.prototype._processReaddir2 = function(prefix, read, abs, remain, index, inGlobStar, entries, cb) {
    ...
    if (!entries)
        return cb(); // It will only match dot entries if it starts with a dot, or if
    ...
    if (len === 0)
        return cb(); // if this is the last remaining pattern bit, then no need for
...
Glob.prototype._readdirInGlobStar = function(abs, cb) {
    if (this.aborted)
        return; // follow all symlinked directories forever
...
Glob.prototype._readdir = function(abs, inGlobStar, cb) {
    ...
    if (!cb)
        return; //console.error('RD %j %j', +inGlobStar, abs)
...
Glob.prototype._readdirEntries = function(abs, entries, cb) {
    if (this.aborted)
        return; // if we haven't asked to stat everything, then just
...
Glob.prototype._readdirError = function(f, er, cb) {
    if (this.aborted)
        return; // handle errors, and cache the information
    switch (er.code) {
...
Glob.prototype._processGlobStar2 = function(prefix, read, abs, remain, index, inGlobStar, entries, cb) {
    ...
    if (!entries)
        return cb(); // test without the globstar, and with every child both below
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (e.charAt(0) === "." && !this.dot)
            continue; // these two cases enter the inGlobStar state
...
Glob.prototype._processSimple2 = function(prefix, index, er, exists, cb) {
    ...
    if (!this.matches[index])
        this.matches[index] = Object.create(null); // If it doesn't exist, then just mark the lack of results
    if (!exists)
        return cb();
    ...
    if (process.platform === "win32")
        prefix = prefix.replace(/\\/g, "/"); // Mark this as a match
...
Glob.prototype._stat = function(f, cb) {
    ...
    if (!this.stat && ownProp(this.cache, abs)) {
        var c = this.cache[abs];
        if (Array.isArray(c))
            c = "DIR"; // It exists, but maybe not how we need it
        if (!needDir || c === "DIR")
            return cb(null, c);
        if (needDir && c === "FILE")
            return cb(); // otherwise we have to stat, because maybe c=true
