...
GlobSync.prototype._processReaddir = function(prefix, read, abs, remain, index, inGlobStar) {
    ...
    if (!entries)
        return; // It will only match dot entries if it starts with a dot, or if
    ...
    if (len === 0)
        return; // if this is the last remaining pattern bit, then no need for
...
GlobSync.prototype._processGlobStar = function(prefix, read, abs, remain, index, inGlobStar) {
    ...
    if (!entries)
        return; // test without the globstar, and with every child both below
...
GlobSync.prototype._stat = function(f) {
    ...
    if (!this.stat && ownProp(this.cache, abs)) {
        ...
        if (!needDir || c === "DIR")
            return c;
        if (needDir && c === "FILE")
            return false; // otherwise we have to stat, because maybe c=true
