...
test("\n\nreading root using function filter", function(t) {
    t.test("\n# file filter -> \"contains root_dir2\"", function(t) {
        t.plan(1);
        readdirp(
            opts({ fileFilter: function(fi) { return fi.name.indexOf("root_dir2") >= 0; } }), function(err, res) {
    ...
    t.test("\n# directory filter -> \"name has length 9\"", function(t) {
        t.plan(1);
        readdirp(
            opts({ directoryFilter: function(di) { return di.name.length === 9; } }), function(err, res) {
...
test("\nprogress callbacks", function(t) {
    ...
    var pluckName = function(fi) { return fi.name; }, processedFiles = [];
...
test("resolving of name, full and relative paths", function(t) {
    var expected = {
        ...
        opts = [
            { root: "./bed", prefix: "" }, { root: "./bed/", prefix: "" }, { root: "bed", prefix: "" }, { root: "bed/", prefix: "" }, { root: "../test/bed/", prefix: "" }, { root: ".", prefix: "bed" }
        ];
    t.plan(opts.length);
    ...
    opts.forEach(function(op) {
        ...
        t.test("\n" + util.inspect(op), function(t) {
            ...
            fs.realpath(op.root, function(err, fullRoot) {
                readdirp(op, function(err, res) {
                    t.equals(
                        res.files[0].fullParentDir, path.join(fullRoot, op.prefix, expected.parentDirName), "correct parentDir"
                    );
                    t.equals(
                        res.files[0].fullPath, path.join(fullRoot, op.prefix, expected.parentDirName, expected.name), "correct fullPath"
                    );
                });
            });
        });
